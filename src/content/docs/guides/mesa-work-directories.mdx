...
<Answer>
    ...
    ```
    <Aside type='caution'>
  If you get an error that `$MESA_DIR` is not set, you need to set it up. Go back to the [prerequisites and setup guide](../prerequisites-and-setup) and ensure you've correctly installed MESA and set the appropriate environment variables, notably `MESA_DIR`, `MESASDK_ROOT`, and `OMP_NUM_THREADS`.
</Aside>
</Answer>
